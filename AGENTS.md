...
- Scheduling execution gating ships as `IJobExecutionAdvisor` (`Continue` fires, `Block` records `Blocked`, `Handle` records `Skipped`). `IJobLifecycleObserver` is notification-only (`OnScheduled`/`OnUnscheduled`/`OnTriggered`/`OnSucceeded`/`OnFailed`/`OnBlocked`/`OnSkipped`; the last two carry default no-op bodies, so existing implementations compile untouched).
