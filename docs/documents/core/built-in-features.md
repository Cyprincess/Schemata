...
| 420,000,000 | `Schemata.Authorization.Foundation` | `SchemataAuthorizationFeature<TApplication, TAuthorization, TScope, TToken>` | `UseAuthorization()`                      | OpenIddict core, server, and validation services; entity stores; ASP.NET Core integration                                                                                                                                                                                   |
...
| 450,000,000 | `Schemata.Resource.Foundation`      | `SchemataResourceFeature`                                                    | `UseResource()`                           | Resource advisor pipeline (validation, freshness, idempotency advisors), `IIdempotencyStore`, auto-discovered `[Resource]` entities                                                                                                                                         |
...
The order of `UseXxx()` calls does not matter -- features are sorted by `Priority` at startup. This means the pipeline is always deterministic regardless of call order.
