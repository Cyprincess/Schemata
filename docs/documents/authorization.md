...
| OIDC Core §5.3/§5.5 | UserInfo endpoint over bearer with the `openid` scope; `acr_values` voluntary satisfiability per §5.5.1.1 (level walk, stronger performed authentication covers a weaker request); `claims` request parameter — `userinfo`/`id_token` members parsed and validated (`essential`/`value`/`values`), a pinned `sub` value mismatch fails the authentication (§5.5.1), an Essential `acr` request cannot be silently downgraded (§5.5.1.1), requested ID-token claims join the token and requested UserInfo claims widen the endpoint output through a private access-token claim, discovery `claims_parameter_supported`; signed and/or encrypted UserInfo responses per §5.3.2 — `userinfo_signed_response_alg`/`userinfo_encrypted_response_alg`/`userinfo_encrypted_response_enc` registration (signing with the issuer's active signing row, encryption with the client's registered `jwks`/`jwks_uri` key material, nested sign-then-encrypt, `iss`/`aud` in signed responses, `application/jwt` content type) | Enforced |
