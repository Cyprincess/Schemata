...
For the full caching subsystem reference — `ICacheProvider`, `DistributedCacheProvider`, `RedisCacheProvider`, and the four advisors — see [caching/query-cache.md](../caching/query-cache.md).
...
| Four cache advisors | `src/Schemata.Entity.Cache/Advisors/` |
...
`UseQueryCache` registers four open-generic scoped advisors:
...
| `AdviceUpdateEvictCache<>` | `IRepositoryUpdateAdvisor<>` | 900,000,000 |
| `AdviceRemoveEvictCache<>` | `IRepositoryRemoveAdvisor<>` | 900,000,000 |
...
| `EvictionEnabled` | `true` | When `false`, update and remove advisors skip eviction; entries live until TTL expires. |
...
| `repository.SuppressQueryCacheEviction()` | `QueryCacheEvictionSuppressed` | Skips `AdviceUpdateEvictCache` and `AdviceRemoveEvictCache` for this instance. |
...
`AdviceUpdateEvictCache` and `AdviceRemoveEvictCache` do not evict immediately. They call `repository.EnqueueAfterCommit(...)` so eviction runs only after the database transaction commits successfully. If the transaction rolls back, the eviction is discarded and the cache retains the pre-mutation entries until TTL expires.
...
- [unit-of-work.md](unit-of-work.md) — `EnqueueAfterCommit` and the after-commit queue
