...
public class AdviceRequestPhoneValidationShould
{
    ...
    private static SchemataUserManager<SchemataUser> MockUserManager(string? phone = "+15551234567") {
        ...
        store.Setup(s => s.FindByIdAsync("42", It.IsAny<CancellationToken>()))
             .ReturnsAsync(new SchemataUser { Uid = Identifiers.NewUid(), UserName = "alice", PhoneNumber = phone });
    ...
    public async Task ThrowsNotFound_WhenUserNotResolved() {
        var advisor = new AdviceRequestPhoneValidation<SchemataUser>(MockUserManager());
