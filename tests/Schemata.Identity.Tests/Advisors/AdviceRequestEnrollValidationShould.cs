...
public class AdviceRequestEnrollValidationShould
{
    ...
    public async Task ThrowsNotFound_WhenUserNotResolved() {
        ...
        var advisor = new AdviceRequestEnrollValidation<SchemataUser>(manager);
    ...
    [Fact]
    public async Task Skips_ForNonEnrollOperations() {
        var manager = MockUserManager();
        var advisor = new AdviceRequestEnrollValidation<SchemataUser>(manager);
        var ctx     = new AdviceContext(null!);
        var request = new AuthenticatorRequest();
