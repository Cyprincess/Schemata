...
public class UserStoreShould
{
    ...
    public async Task DeleteUser_RemovesAllDependentRowsAtomically() {
        ...
        users.Setup(r => r.RemoveAsync(It.IsAny<SchemataUser>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        userRole.Setup(r => r.ListAsync(
                           It.IsAny<Func<IQueryable<SchemataUserRole>, IQueryable<SchemataUserRole>>?>(),
                           It.IsAny<CancellationToken>()))
                .Returns(OneAsync(link));
        userRole.Setup(r => r.RemoveAsync(It.IsAny<SchemataUserRole>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        claims.Setup(r => r.ListAsync(
                         It.IsAny<Func<IQueryable<SchemataUserClaim>, IQueryable<SchemataUserClaim>>?>(),
                         It.IsAny<CancellationToken>()))
              .Returns(OneAsync(claim));
        claims.Setup(r => r.RemoveAsync(It.IsAny<SchemataUserClaim>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        logins.Setup(r => r.ListAsync(
                         It.IsAny<Func<IQueryable<SchemataUserLogin>, IQueryable<SchemataUserLogin>>?>(),
                         It.IsAny<CancellationToken>()))
              .Returns(OneAsync(login));
        logins.Setup(r => r.RemoveAsync(It.IsAny<SchemataUserLogin>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        tokens.Setup(r => r.ListAsync(
                         It.IsAny<Func<IQueryable<SchemataUserToken>, IQueryable<SchemataUserToken>>?>(),
                         It.IsAny<CancellationToken>()))
              .Returns(OneAsync(token));
        tokens.Setup(r => r.RemoveAsync(It.IsAny<SchemataUserToken>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        var store = new SchemataUserStore<SchemataUser>(
            users.Object, roles.Object, claims.Object, userRole.Object, logins.Object, tokens.Object);

        var result = await store.DeleteAsync(user, CancellationToken.None);

        Assert.True(result.Succeeded);

        // Every dependent repository enlists in the user's unit of work and its rows are removed
        // before the user, so no orphaned child rows survive the delete.
