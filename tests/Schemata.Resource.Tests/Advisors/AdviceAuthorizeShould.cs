...
public class AdviceAuthorizeShould
{
    ...
    public async Task MethodRequest_CanonicalName_IsUsedForPermissionErrorResource() {
        ...
        Assert.Equal(request.CanonicalName, resource.ResourceName);
        ...
        Assert.Equal("archive", captured.Operation);
    }
    ...
    public async Task MethodRequest_Anonymous_AppliesEntitlementWithoutAccessCheck() {
        ...
        var result = await advisor.AdviseAsync(ctx, new(), container, null);

        Assert.Equal(AdviseResult.Continue, result);
        var filtered = container.Query(new[] {
            new Student { FullName = "Allowed" }, new Student { FullName = "Denied" },
        }.AsQueryable()).ToArray();
        Assert.Single(filtered);
        Assert.Equal("Allowed", filtered[0].FullName);
    }
    ...
    public async Task Create_WhenPrimaryDeniedButParentGranted_ThrowsAuthorizationWithTemplate() {
        var access = new Mock<IAccessProvider<Student, Student>>();
        access.Setup(a => a.HasAccessAsync(It.IsAny<Student?>(),
    ...
    public async Task Get_UnauthorizedButParentVisible_DisclosesPermission() {
        ...
        var access = new Mock<IAccessProvider<Student, GetRequest>>();
        access.SetupSequence(a => a.HasAccessAsync(It.IsAny<Student?>(), It.IsAny<AccessContext<GetRequest>>(),
                                                   It.IsAny<ClaimsPrincipal?>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync(false)
              .ReturnsAsync(true);
    ...
    public async Task List_UnauthorizedAndParentHidden_ThrowsNotFound() {
        ...
        access.Setup(a => a.HasAccessAsync(It.IsAny<Student?>(), It.IsAny<AccessContext<ListRequest>>(),
                                           It.IsAny<ClaimsPrincipal?>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync(false);
    ...
    public async Task List_UnauthorizedButParentVisible_DisclosesPermissionOnParent() {
        var access = new Mock<IAccessProvider<Student, ListRequest>>();
        access.Setup(a => a.HasAccessAsync(It.IsAny<Student?>(),
                                           It.Is<AccessContext<ListRequest>>(c => c.Operation
                                                                               == nameof(Operations.List)),
                                           It.IsAny<ClaimsPrincipal?>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync(false);
        access.Setup(a => a.HasAccessAsync(It.IsAny<Student?>(),
                                           It.Is<AccessContext<ListRequest>>(c => c.Operation
                                                                               == nameof(Operations.Get)),
                                           It.IsAny<ClaimsPrincipal?>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync(true);
    ...
    public async Task Delete_UnauthorizedAndParentHidden_ThrowsNotFound() {
        ...
        access.Setup(a => a.HasAccessAsync(It.IsAny<Student?>(), It.IsAny<AccessContext<DeleteRequest>>(),
                                           It.IsAny<ClaimsPrincipal?>(), It.IsAny<CancellationToken>()))
              .ReturnsAsync(false);
