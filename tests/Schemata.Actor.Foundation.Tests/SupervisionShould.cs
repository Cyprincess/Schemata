...
public class SupervisionShould
{
    ...
    public async Task OnFailedAsync_ReturningTrue_RestartsTheActor_AndTheNextMessageIsStillProcessed() {
        ...
        var actor           = await system.SpawnAsync(new ActorId("supervised", "restart"), new Props(typeof(SupervisedActor), [true]));
        ...
        var count = await actor.AskAsync<Increment, int>(new Increment());
    ...
    public async Task OnFailedAsync_ReturningFalse_StopsTheActor_AndAFreshInstanceIsSpawnedNext() {
        ...
        registry.Register("supervised", new Props(typeof(SupervisedActor), [false]));
        var id     = new ActorId("supervised", "stop");
        var actor  = await system.GetAsync(id);
        var originalId      = await actor.AskAsync<WhoAmI, Guid>(new WhoAmI());
        ...
        var failing = actor.AskAsync<Fail, string>(new Fail("die")).AsTask();
        ...
        var freshId = await fresh.AskAsync<WhoAmI, Guid>(new WhoAmI());
