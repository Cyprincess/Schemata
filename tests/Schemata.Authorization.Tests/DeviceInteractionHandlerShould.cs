...
public class DeviceInteractionHandlerShould
{
    private static Fixture CreateFixture() {
        ...
        apps.Setup(a => a.FindByCanonicalNameAsync("device-client", It.IsAny<CancellationToken>()))
            .ReturnsAsync(app);

        var devicePayload = JsonSerializer.Serialize(new DeviceCodePayload {
        ...
        var userCodePayload = JsonSerializer.Serialize(new UserCodePayload {
            DeviceCodeName = device.Name,
            Scope          = "openid profile",
            ClientId       = "device-client",
        }, jsonOpts.Value);
        ...
        tokens.Setup(t => t.FindByReferenceIdAsync("user-ref", It.IsAny<CancellationToken>()))
              .ReturnsAsync(userCode);
        tokens.Setup(t => t.FindByCanonicalNameAsync(device.Name!, It.IsAny<CancellationToken>()))
              .ReturnsAsync(device);
        ...
        var handler = new DeviceInteractionHandler<SchemataApplication, SchemataAuthorization, SchemataScope, SchemataToken>(
    ...
    private static ClaimsPrincipal CreatePrincipal(string subject = "user-42", string sid = "sess-99") {
        return new(new ClaimsIdentity(
                       [new Claim(Claims.Subject, subject), new Claim("sid", sid)],
                       "test"));
    }
    ...
    public async Task CreatesAuthorization_AndBindsToDeviceToken_OnApprove() {
        ...
        await Assert.ThrowsAsync<NoContentException>(
            () => f.Handler.ApproveAsync(new InteractRequest { Code = "user-ref" }, principal, "https://auth", CancellationToken.None));
