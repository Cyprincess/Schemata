...
public class IsolationShould : IAsyncLifetime
{
    ...
    public async Task TwoRepositoriesInSameScope_HaveIndependentChangeTrackers() {
        ...
        await a.AddAsync(new() { FullName = "Iso-Alice", Age = 18, Grade = 1, Name = "iso-alice" });

        // b has no pending work in its own change tracker; CommitAsync is a no-op.
        await b.CommitAsync();
    ...
    public async Task EnumerateOnOneRepo_ConcurrentWriteOnAnother_DoesNotThrow() {
        {
            ...
            for (var i = 0; i < 5; i++)
                await seedRepo.AddAsync(new() { FullName = $"Seed-{i}", Age = 18+i, Grade = 1, Name = $"seed-{i}" });
        ...
        await foreach (var student in reader.ListAsync<Student>(q => q.OrderBy(s => s.Name))) {
            await writer.AddAsync(new() {
                FullName = $"Side-{student.Name}", Age = 30, Grade = 9, Name = $"side-{student.Name}",
            });
        }
