...
public class ProcessDefinitionQueryServiceShould
{
    [Fact]
    public void ListDefinitions_ProjectsElementGraphWithPerEventTriggers() {
        ...
        var service = new ProcessDefinitionQueryService(registry.Object);

        var info = Assert.Single(service.ListProcessDefinitions());
    ...
    [Fact]
    public void ListDefinitions_LeavesGraphEmptyWhenRegistrationIsMissing() {
        var registry = new Mock<IProcessRegistry>();
        registry.Setup(r => r.GetRegisteredProcesses()).Returns(["orders"]);
