...
public class FlowCustomMethodGrpcShould : IClassFixture<WebAppFactory>
{
    private readonly WebAppFactory _factory;

    public FlowCustomMethodGrpcShould(WebAppFactory factory) { _factory = factory; }
    ...
    public async Task CompleteProcess_Missing_Instance_Returns_NotFound() {
        var error = await Assert.ThrowsAsync<RpcException>(() =>
            Call<SchemataProcess, CompleteActivityRequest, ProcessSnapshot>(
    ...
    public async Task CorrelateProcess_Missing_Instance_Returns_NotFound() {
        var error = await Assert.ThrowsAsync<RpcException>(() =>
            Call<SchemataProcess, CorrelateMessageRequest, ProcessSnapshot>(
    ...
    public async Task SignalProcess_Empty_Broadcast_Returns_Response() {
        var response = await Call<SchemataProcess, ThrowSignalRequest, EmptyResourceResponse>(
