...
- `Schemata.Common` — shared value types, JSON options, hashing, predicate helpers. Depends on `Abstractions`.
...
- `Foundation` also pulls `Event.Skeleton` + `Entity.Repository`
- `Http` pulls `Resource.Http`; `Grpc` pulls `Resource.Grpc`
- `Event` pulls `Event.Foundation`; `Scheduling` pulls `Scheduling.Foundation`
