...
- **A repository reopens its own UoW on commit; a `Begin()`/`Join()` one never reopens.** The implicit UoW enlisted by a standalone write is reset by `CommitAsync`, so one injected `IRepository<T>` serves repeated write/commit cycles in a scope — inject it, never re-resolve it from `IServiceProvider` to dodge a completed UoW. A caller-supplied UoW completes for good: call `Begin()` again or resolve a fresh repository. `Begin()` always creates a new UoW rather than exposing the implicit one.
