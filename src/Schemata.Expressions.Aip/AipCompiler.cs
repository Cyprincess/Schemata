...
public sealed class AipCompiler : IExpressionCompiler
{
    ...
    public IExpressionTree Parse(string source) {
        ...
        return ExpressionCache.GetOrAddTree(key, () => AipParser.Filter.Parse(source) ?? throw new ArgumentException("Invalid AIP filter.", nameof(source)));
    }
    ...
    public Expression<Func<TContext, TResult>> Compile<TContext, TResult>(
        ...
        var key = ExpressionCacheKey.Create(Language, filter.ToString() ?? string.Empty, typeof(TContext), typeof(TResult), AipBuiltInFunctions.Fingerprint(options));
