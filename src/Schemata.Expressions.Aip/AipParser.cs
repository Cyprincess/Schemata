...
public static class AipParser
{
    ...
    static AipParser() {
        ...
        var @string = Parsers.Terms.Pattern(c => Character.IsIdentifierPart(c) || c > 127)
                             .When((_, span) => {
                                  var s = span.Span.ToString();
        ...
        var keyword = Parsers.Terms.Pattern(c => Character.IsIdentifierPart(c) || c > 127)
                             .When((_, span) => {
                                  var s = span.Span.ToString();
                                  return string.Equals(s, "AND", StringComparison.OrdinalIgnoreCase)
                                      || string.Equals(s, "OR", StringComparison.OrdinalIgnoreCase)
        ...
        var args = arg.And(Parsers.ZeroOrMany(comma.SkipAnd(arg))).Then(a => a.Item2.Prepend(a.Item1).ToArray());
