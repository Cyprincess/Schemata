...
///         Reverse pairwise lookup resolves through the persisted subject-mapping store;
///         the hash itself is one-way, so a value absent from the store (never issued
///         through OIDC on any node sharing the database) yields <see langword="null" />.
///     </para>
/// </remarks>
