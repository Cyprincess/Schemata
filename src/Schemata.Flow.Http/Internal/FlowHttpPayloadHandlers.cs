...
internal sealed class FlowHttpCorrelateMessageHandler(FlowRunner runner, IProcessRegistry registry)
    : IResourceMethodHandler<SchemataProcess, CorrelateMessageRequest, ProcessSnapshot>
{
    public ValueTask<ProcessSnapshot> InvokeAsync(
        ...
        var payload = FlowHttpPayload.Deserialize(request.Payload, reg.MessagePayloadTypes.GetValueOrDefault(request.MessageName));
        return runner.CorrelateAsync(entity, request.MessageName, payload, request.Token, principal, ct);
    }
}

internal sealed class FlowHttpThrowSignalHandler(FlowRunner runner, IProcessRegistry registry)
    : IResourceMethodHandler<SchemataProcess, ThrowSignalRequest, EmptyResourceResponse>
{
    public async ValueTask<EmptyResourceResponse> InvokeAsync(
        ...
        var payload = FlowHttpPayload.Deserialize(request.Payload, SignalPayloadType(registry, request.SignalName));
...
internal static class FlowHttpPayload
{
