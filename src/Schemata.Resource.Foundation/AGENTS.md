...
- `IResourceRegistry` / `ResourceRegistry`: the registered resources and their AIP-136 custom methods. **`SchemataResourceBuilder` owns the instance**: its constructor creates the registry on the first builder, stores it in the `SchemataOptions` bag under `Schemata.Resource.Registry` and registers it as the `IResourceRegistry` singleton, so the builders Flow / Report / Scheduling each construct for their own resources all accumulate into one registry without scanning `IServiceCollection` for it. The `AddResource` extension is `internal` and takes that registry — registration reaches it through the builder or not at all. The registry **seals on its first read**, which is why `DefaultResourceTypeResolver` can build its indexes in the constructor. `SchemataResourceOptions` carries configuration only — never the resource set.
...
- Authentication scheme resolves per resource, not globally: `ResourceAttribute.AuthenticationScheme` wins, else `SchemataResourceOptions.AuthenticationScheme` (set by `WithAuthorization(scheme)`), else no authorization policy is attached at all. `SchemataResourceBuilder.AuthenticationScheme` stamps every resource **that builder** registers, which is how Flow / Report / Scheduling carry their own `WithAuthorization(scheme)` down to their own resources without touching the global default.
